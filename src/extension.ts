...
export function activate(context: vscode.ExtensionContext) {
  let app = vscode.commands.registerCommand("errorresolver.display", () => {
	...
	<head>
		...
		<style>
			...
			.question-div {
				...
				box-shadow: 10px 7px 38px 0px rgba(232, 232, 232, 1)!important;
			}
	...
	<body>
		...
		<script>
			...
			function createAnswerCard(answers, question) {
				let ans = answers.slice(0, 1);
				let cards = '';
				for (let i in ans) {
					cards += '<div class="shadow card"><div class="question-div"><a target="_blank" href="' + question.link + '"><h2>Query : ' + question.title + '</h2></a>' + question.body + ' <span style="position:absolute;top:5px;left:10px;font-size:20pt;opacity:.1;font-weight:bold;color:#ddd;">' + (Number(i) + 1) + '</span></div><div style="padding:20px;">' + ans[i].body + '</div></div>';
			...
			function createQuestionsCard(questions) {
				let cards = '<h2>Related Questions</h2>';
				let i = 0;
